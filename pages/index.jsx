...
export default function Home() {
  const [event, setEvent] = useState('开学危机');
  const [time, setTime] = useState('8');
  const [unit, setUnit] = useState('小时');
  const [english, setEnglish] = useState('8 hours left before the school crisis');
    ...
    <div className="max-w-4xl mx-auto px-6 sm:px-0 py-24">
      ...
      <div className='flex flex-row justify-between text-sm opacity-75 mb-4'>
        ...
        <div className='flex flex-row space-x-2'>
          <p className='inter font-medium'>V0.21 Alpha</p>
      ...
      <div className='my-6 grid grid-cols-1 sm:grid-cols-3 gap-4'>
        <div className='w-full col-span-2 overflow-x-auto bg-black text-white rounded-3xl min-h-[16rem] sm:min-h-[24rem] px-8 sm:px-36 py-24'>
          <div ref={domRef} id="node"
            className={cn(
              scale === '' ? 'scale-[0.75] sm:scale-100' : `scale-[0.${scale}]`,
              bg === 'false' ? '' : 'bg-black'
            )}>
            <h2 className='text-4xl mb-3 word -ml-8'>距{event}</h2>
            <div className='grid grid-cols-3 grid-rows-2'>
              ...
              <div className='col-span-2 -ml-4 sm:-ml-7'>
                <div className='flex flex-row word flex-nowrap'>
                  <h2 className='text-4xl'>还剩</h2>
                  <h2 className='text-8xl text-red-600 -mt-10 font-bold ml-1 mr-1 inter'>{time}</h2>
        ...
        <div className='flex flex-col space-y-3 px-0 sm:px-1'>
          ...
          <div className='flex flex-col space-y-2 opacity-90'>
            ...
            <input onChange={(e) => setScale(e.target.value)} value={scale} className='inter w-full rounded-lg px-4 py-2 bg-zinc-100 border border-zinc-200' placeholder='调整缩放适应屏幕(接受数字0-100)' />
          ...
          <div>
            <p className='text-xs opacity-50'>1.英文翻译可以从百度翻译等网站获取</p>
            <p className='mt-1 text-xs opacity-50'>2.导出时间较长，请耐心等待。具体导出时间取决于设备配置。</p>
          </div>
        </div>
      </div>
